...
class OptionsService {
	...
	_defineDirectoryName(options) {
		return path.normalize(`${options.pathToLogsFolder}/${options.env}/${options.logType}`);
	}

	_defineFileName(options) {
		return `${options.fileNamePrefix}${dateFormat(new Date(), options.fileDateFormat)}${options.fileNameExtension}`;
	}
