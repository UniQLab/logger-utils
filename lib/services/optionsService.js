...
const transactionConfig = require('../config/transactions').OPTIONS;
...
class OptionsService {
	constructor(userOptions, loggerOptions) {
		...
		if (this.options.logIntoFile && !this.options.pathToLogsFolder) throw new Error(errorConfig.LOG_INTO_FILE_CONFLICT);
		...
		const filesMeta = optionsUtils.moveOptions(this.options, transactionConfig.FILES.FROM, transactionConfig.FILES.TO);
		const foldersMeta = optionsUtils.moveOptions(this.options, transactionConfig.FOLDERS.FROM, transactionConfig.FOLDERS.TO);
		...
		optionsUtils.moveOptions(this.options,
			transactionConfig.FOLDERS.FROM,
			transactionConfig.FOLDERS.TO
		)
