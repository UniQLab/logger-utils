...
class TransportStore {
	...
	add(transport) {
		...
		if (_.isFunction(transport) && !this.exists(transport)) this.transports.push(transport);
		else _.each(transport, single => {
			if (!this.exists(transport)) this.transports.push(single)
		});
